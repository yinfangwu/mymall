...
public class OrderServiceImpl implements OrderService {
    ...
    public ServerResponse pay(Integer userId, Long orderNo, String path) {
        ...
        if (order == null){
            return ServerResponse.createByErrorMessage("找不到订单");
        }
        ...
        String subject = new StringBuilder().append("mymall商城扫码支付，订单号：").append(outTradeNo).toString();
        ...
        String operaterId = "test_operater_id";
        ...
        for(OrderItem orderItem : orderItemList){
            GoodsDetail goodsDetail = GoodsDetail.newInstance(order.getOrderNo().toString(), orderItem.getProductName(),
                    BigDecimalUtil.add(orderItem.getCurrentUnitPrice().doubleValue(), new Double(100)).longValue(),
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setOperatorId(operaterId).setStoreId(storeId).setExtendParams(extendParams)
    ...
    @Override
    public ServerResponse<PageInfo> getOrderList(Integer useId, int pageNum, int pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        List<Order> orderList = orderMapper.selectByUserId(useId);
        PageInfo pageInfo = new PageInfo(orderList);
        List<OrderVo> orderVoList = this.assembleOrderVoList(useId, orderList);
     ...
     */
    @Override
    public ServerResponse<OrderVo> getManageSendGoods(Long orderNo) {
        Order order = orderMapper.selectByOrderNo(orderNo);
        if (order != null){
            if (order.getStatus() == Const.OrderStatusEnum.PAID.getCode()){
